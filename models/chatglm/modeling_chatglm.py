...
def load_tf_weights_in_chatglm_6b(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        pointer = model
        for m_name in name:
            scope_names = re.split("_(\\d+)", m_name) if re.fullmatch("[A-Za-z]+_\\d+", m_name) else [m_name]
...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        outputs = (output,) + outputs if use_cache else (output,) + outputs[1:]
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    all you need](https://arxiv.org/abs/1706.03762) by Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion
    Jones, Aidan N.
        ...
        if past is not None or past_key_values is not None:
            ...
            if position_ids is not None:
                ...
                if self.position_encoding_2d:
                    ...
                    position_ids = torch.tensor(
                        list(mask_positions), dtype=torch.long, device=input_ids.device
                    ).unsqueeze(-1)
