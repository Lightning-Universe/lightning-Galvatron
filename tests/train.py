...
Because Galvatron uses megatron to initialize the distribution environment,
and we cannot find a way to completely clean the state between tests using only one process.
"""
...
from lightning_galvatron import GalvatronStrategy, GalvatronEnvironment
...
try:
    ...
    class GPTModel(LightningModule):
        def __init__(self, strategy_options):
            ...
            Args = namedtuple("Args", ["overwrite_config", "model_size", "vocab_size", "hidden_size", "num_hidden_layers", "seq_length", "use_flash_attn"])
            self.args = Args(
        ...
        def train_dataloader(self) -> TRAIN_DATALOADERS:
            ...
            trainloader = DataLoader(dataset=dataset,
                                    ...
                                    sampler=DistributedSampler(dataset,shuffle=True,num_replicas=data_num_replicas,rank=self.trainer.local_rank%data_num_replicas))
            return trainloader
except:
...
class ChatGLMModel(LightningModule):
    def __init__(self, strategy_options):
        ...
        self.config = AutoConfig.from_pretrained(os.path.join(working_dir, os.pardir, "models", "chatglm"), trust_remote_code=True)
        # self.config = AutoConfig.from_pretrained("THUDM/chatglm-6b", trust_remote_code=True)
        Args = namedtuple("Args", ["vocab_size", "hidden_size", "num_hidden_layers", "num_attention_heads", "seq_length"])
        self.args = Args(
...
def run_test_from_config(trainer_options, strategy_options):
    ...
    trainer = Trainer(
        strategy=GalvatronStrategy(**strategy_options),
        **trainer_options
    )
    trainer.fit(model)
